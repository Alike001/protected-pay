...
function ActivePayment({ onUndo }: { onUndo: () => void }) {
  ...
  const terminalRedacted = Boolean(livePayment.payment?.redacted);
  const activeAmount = livePayment.payment && !livePayment.payment.redacted ? formatUsdc(livePayment.payment.amount) : restoredReceipt ? formatUsdc(BigInt(restoredReceipt.amount)) : amount;
  const activeRecipient = livePayment.payment && !livePayment.payment.redacted ? livePayment.payment.recipient : restoredReceipt?.counterparty ?? recipient;
  ...
  return (
    <main className="dashboard-main" id="dashboard">
      ...
      {preview && !restoredReceipt ? <ActivePayment onUndo={() => setUndoing(true)} /> : recovered ? (
        ...
        <section className="panel confirmed-payment"><span className="icon-tile success">{canRecoverExpired ? <RotateCcw size={19} /> : <Check size={19} />}</span><div><h2>{terminalRedacted ? livePayment.payment?.status === PaymentStatus.Expired ? "Payment recovered" : "Payment claimed" : canRecoverExpired ? "Payment expired safely" : receipt ? "Payment protected" : "Payment restored"}</h2><p>{terminalRedacted ? livePayment.payment?.status === PaymentStatus.Expired ? "The expired test USDC returned to your protected balance." : "The recipient claimed the test USDC and the private payment fields were erased." : canRecoverExpired ? "The recipient did not acknowledge in time. Recover the test USDC to your protected balance." : ` ${activeAmount} test USDC to ${shortAddress(activeRecipient, 7)}. Share this private-view link with the intended recipient.`}</p>{!terminalRedacted && recipientLink && <code>{recipientLink}</code>}{cancelError && <p className="workflow-error" role="alert">{cancelError}</p>}</div>{!terminalRedacted && <div className="confirmed-actions"><button className="secondary-action" onClick={() => recipientLink && navigator.clipboard.writeText(recipientLink)} disabled={!recipientLink}>Copy link</button>{canUndoActive && <button className="undo-button" onClick={() => setUndoing(true)}><RotateCcw size={16} /> Undo</button>}{canRecoverExpired && <button className="undo-button" onClick={recoverExpiredPayment} disabled={canceling}><RotateCcw size={16} /> {canceling ? "Recovering…" : "Recover expired"}</button>}</div>}</section>
      ) : (
