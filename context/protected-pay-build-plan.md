...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / LIVE AUTONOMOUS EXPIRY PASS | Expiry fixture reached `Expired` at `+300s`; sender-only claim restores escrow and redacts terms in signed simulation without persistence | Approve live claim, prove retry idempotency, then repeat privacy/commit checks |
...
8. **In progress:** the corrected settlement path is complete, and a separate deterministic expiry fixture autonomously reached `Expired` at the sixth validator-signed execution with no acknowledgement. Its sender-only claim passes signature-verified simulation and persists nothing. Next, separately approve live claim broadcast, verify the 1 test-USDC refund plus terminal redaction, and prove retries cannot refund or mutate twice.
