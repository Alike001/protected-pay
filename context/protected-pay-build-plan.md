...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / EXPIRY DELEGATED | Corrected settlement is complete; distinct expiry shell and permission are delegated without value movement | Privately open and schedule it without acknowledgement, then prove unattended expiry and sender recovery live |
...
8. **In progress:** the corrected settlement path is complete. A separate deterministic expiry shell and permission are delegated on Devnet with no value movement. Next, authenticate the sender and simulate a private atomic open with the six-call schedule; after separate broadcast approval, omit acknowledgement, observe automatic expiry at call six, let only the sender recover escrow, and prove retries are idempotent.
