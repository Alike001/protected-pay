...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 LOCAL FIX PASS, UPGRADE PENDING | Atomic private escrow and Payment-only scheduling finalized; six-call correction and regression coverage pass locally | Simulate and approve the compatible upgrade, then prove acknowledge/settle/claim and expire/recover paths |
...
4. **Next:** run a signature-verified, non-broadcast simulation of the exact Devnet program upgrade; broadcast only after separate explicit approval.
5. Recover the current safe-but-`Created` escrow through an explicitly approved manual `advance_payment` followed by the sender-only claim path.
6. Prove the corrected real Crank lifecycle twice on fresh fixtures: acknowledged payment to settlement and abandoned payment to automatic expiry/refund, with both users offline and retries idempotent.
7. Repeat the privacy audit and commit/undelegate proof for the corrected Payment layout. Preserve the narrow claim: pending amount, memo hash, status, and balances are private; wallet relationships and delegation metadata are not anonymous.
8. Measure the funded-user approval count and package the normal flow as simply as the infrastructure permits.
9. Build the product UI: fund, send, share, acknowledge, countdown, Undo, activity, withdrawal, recovery, plus a separate judge-proof view.
10. Run all three end-to-end stories from fresh state, perform the final verification audit, deploy the web client, record the product-first video, confirm the authenticated submission deadline, and submit.
