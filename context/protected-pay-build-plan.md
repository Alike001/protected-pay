...
Status: active implementation plan. G0–G3 passed with narrowed privacy language. Phase 4's real private open and sender recovery paths pass, and the version-2 permission-topology correction is deployed on Devnet. The first atomic Payment schedule exposed an off-by-one cadence defect because MagicBlock executes the first iteration immediately. Version 2.1 corrects the program and client to six 60-second iterations, passes the complete local gate, and is deployed and independently verified on Devnet. The escrow stranded by the old cadence is atomically recovered and redacted. Both corrected live paths now pass end to end: acknowledged settlement pays the recipient, while unattended expiry automatically returns entitlement to the sender and the sender-only claim recovers escrow. Live retry safety, the corrected-layout privacy audit, and signature-verified terminal closeout simulation pass. Only the separately approved commit/undelegate broadcast and public post-commit verification remain before product UI work.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / BOTH E2E PATHS PASS / RETRY SAFETY PASS / CLOSEOUT SIMULATION PASS | Settlement, unattended expiry, owner-only recovery, terminal no-op, duplicate-claim rejection, corrected-layout privacy boundary, and terminal closeout simulation proven against live state | Separately approve broadcast, then verify public redacted bytes |
...
9. **IN PROGRESS:** The corrected-layout public/unauthenticated privacy audit and sender-authenticated signature-verified simulation pass. The exact 320-byte Payment-only closeout used 39,402 compute units, preserved the redacted terminal bytes and commitment, moved no USDC, excluded both Deposits and the Permission, leaked neither original amount nor memo hash in logs, and persisted nothing. Next separately approve broadcast and verify the final public redacted bytes, closed Payment delegation record/metadata, still-delegated aggregate Deposits, and unchanged vault collateral.
