...
Status: active implementation plan. G0–G3 and Phase 4 are complete with narrowed privacy language. Version 2.1 is deployed, both corrected live paths pass end to end, retry safety passes, and the corrected Payment passed its field-by-field privacy audit through finalized public redacted commitment. The local Phase 5.1 upgrade now replaces direct Phantom signatures on Private ER actions with a one-hour, program-scoped MagicBlock Session Token and in-memory signer; explicit revocation is wired into the sender UI. Cold sending still honestly requires two public transaction approvals plus one Query Filtering login message, but the private open/schedule transaction is session-signed without Phantom's incompatible base-layer simulation. The new 677,280-byte binary, Rust tests, clippy, generated client, TypeScript, browser packaging, web build, and unsigned live Session Keys simulation pass. It is not deployed: ProgramData needs a 65,536-byte extension, followed by separately approved signed upgrade simulation and broadcast. Live browser-wallet evidence resumes only after that upgrade.
...
- Fiat settlement and Resolva integration remain deferred. Session-key authorization is implemented locally but remains undeployed pending the explicit upgrade gates.
...
| Phase 5 | IN PROGRESS | React/Vite product, Wallet Standard connection, retry-safe custody/payment workflows, and locally verified one-hour MagicBlock Session Tokens with in-memory private signers and explicit revocation | Extend ProgramData, upload and simulate the exact authorization upgrade, deploy after separate approval, then complete live browser-wallet proof |
...
11. **In progress, deployment gated:** the product UI now covers its public landing page, connect, protected-balance unlock/read, first and subsequent test-USDC funding, retry-safe withdrawal, real funded-sender send/share/Undo/expired recovery, authoritative recipient Payment read/countdown, first-time recipient onboarding, real acknowledgement/claim, encrypted link-carried notes, cross-tab recovery, activity layout, judge proof, bounded-session expiry, and explicit revocation. The 677,280-byte authorization binary exceeds the deployed 635,136-byte capacity. Extend by 65,536 bytes, create/upload a byte-verified buffer, run the exact signed non-broadcast upgrade simulation, then request separate broadcast approval before resuming live browser-wallet evidence.
...
12. After the program upgrade, run all three end-to-end stories from fresh state, including session-signed success, explicit revocation denial, and expiry denial; perform the final verification audit, deploy the web client, record the product-first video, confirm the authenticated submission deadline, and submit.
