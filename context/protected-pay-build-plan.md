...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / CORRECTED SETTLEMENT E2E PASS | Corrected Crank schedule autonomously settled the acknowledged Payment and recipient claim finalized with terminal redaction | Prove a fresh unattended expiry and sender recovery live |
...
8. **In progress:** the corrected settlement path is complete: the six-call schedule autonomously advanced `Acknowledged -> Settled`, and the recipient claim finalized, credited 1 test USDC, and redacted the Payment. Next, repeat on a new fixture without acknowledgement to prove automatic expiry and sender recovery, with retries idempotent.
