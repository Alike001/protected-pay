...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED | Atomic private escrow and Payment-only scheduling finalized; six-call correction passes locally and its exact binary is deployed and independently verified on Devnet | Recover the existing escrow, then prove both corrected live terminal paths |
...
7. **Next:** recover the current safe-but-`Created` escrow through an explicitly approved manual `advance_payment` followed by the sender-only claim path.
