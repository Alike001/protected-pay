...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH DELEGATION SIMULATION PASS | Six-call correction is deployed; old escrow is recovered; fresh settlement shell/permission are live and delegation simulation passes | Separately approve delegation broadcast, then prove settlement and expiry live |
...
8. **In progress:** the fresh acknowledged-payment shell and permission are finalized on base Devnet, and their unsigned delegation simulation passes without including either private Deposit. Next, separately approve the already-preflighted delegation broadcast, then prove private open plus six-call scheduling, acknowledgement, automatic settlement, and recipient claim. Repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
