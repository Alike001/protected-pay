...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH PAYMENT ACKNOWLEDGED LIVE | Corrected six-call Payment is privately open and recipient-acknowledged; privacy boundary remains intact | Observe automatic settlement, claim as recipient, then prove fresh expiry live |
...
8. **In progress:** the fresh Payment is privately open with the corrected six-call schedule and the recipient acknowledgement finalized before the first 60-second boundary. Next, authenticate the recipient to verify autonomous `Acknowledged -> Settled`, then separately simulate and broadcast the recipient claim. Repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
