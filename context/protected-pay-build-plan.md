...
- A fresh version-2 settlement Payment shell and permission are finalized on Devnet. Their Deposit-free delegation passes unsigned simulation with unchanged financial bytes and awaits its own broadcast approval.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2 DEPLOYED | Real v1 private open and recovery pass; v2 per-Payment escrow and a fresh settlement shell are live; safe delegation is simulated | Broadcast delegation, then prove settlement/expiry/claim/cancel-race paths |
...
3. **Next:** after explicit approval, broadcast the simulated delegation of only the fresh version-2 settlement Payment and permission.
