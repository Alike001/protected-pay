...
- A fresh version-2 settlement Payment shell and permission are finalized on Devnet. No USDC moved and the existing delegated Deposits remained unchanged; delegating only the new Payment and permission is the next checkpoint.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2 DEPLOYED | Real v1 private open and recovery pass; v2 per-Payment escrow and a fresh settlement shell are live | Delegate the new shell, then prove settlement/expiry/claim/cancel-race paths |
...
3. **Next:** simulate delegation of only the fresh version-2 settlement Payment and permission; reuse the already-delegated user Deposits.
