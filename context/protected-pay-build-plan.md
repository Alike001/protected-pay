...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH SETTLEMENT DELEGATED | Six-call correction is deployed; old escrow is recovered; fresh settlement Payment/permission are delegated on Devnet | Prove the corrected settlement and expiry lifecycles live |
...
8. **In progress:** the fresh acknowledged-payment shell and permission are delegated on Devnet without including or mutating either private Deposit. Next, authenticate the sender and prove private open plus six-call scheduling in signed simulation, then separately broadcast, acknowledge as the recipient, observe automatic settlement, and claim. Repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
