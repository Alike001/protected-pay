...
| Phase 5 | IN PROGRESS | React/Vite surfaces, Wallet Standard connection, validated private sessions, first balance setup, real funded-sender open/Undo/expired recovery, first-time recipient onboarding, recipient read/acknowledge/claim, encrypted recipient links, same-tab reload restoration, proof drawer, responsive QA | Withdrawal, subsequent top-ups, reconciliation edge cases, live browser-wallet proof, and remaining error states |
...
11. **In progress:** the product UI now covers connect, protected-balance unlock/read, first test-USDC setup, real funded-sender send/share/Undo/expired recovery, authoritative recipient Payment read/countdown, first-time recipient onboarding, real acknowledgement/claim, encrypted link-carried notes, same-tab reload restoration, activity layout, and judge proof. Unsigned builders also package private balance return in 320 bytes and public top-up/withdrawal plus re-delegation in 737 bytes. The remaining work is retry-safe orchestration before those controls are exposed, full terminal-state reconciliation, live browser-wallet evidence, and denied/delayed/rejected-signature variants.
