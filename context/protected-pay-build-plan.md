...
- The version-2.1 exact-upgrade preflight found no authority-owned Devnet buffer. ProgramData already has 635,136 bytes of capacity, so no extension is needed; a new 633,605-byte loader buffer requires 3.21936364 Devnet SOL of refundable rent. No upgrade transaction was constructed or signed because an exact loader simulation cannot consume a nonexistent buffer.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 LOCAL FIX PASS, BUFFER REQUIRED | Atomic private escrow and Payment-only scheduling finalized; six-call correction and regression coverage pass locally | Upload the reviewed buffer, simulate and approve the compatible upgrade, then prove both live terminal paths |
...
4. **Next:** create and fully upload a fresh Devnet loader buffer containing exactly the reviewed version-2.1 binary; this temporarily locks 3.21936364 Devnet SOL and requires separate approval.
5. Run a signature-verified, non-broadcast simulation of the exact Devnet program upgrade; broadcast only after another explicit approval.
