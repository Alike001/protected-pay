...
- The fresh version-2.1 acknowledged-payment fixture derives Payment `71t8qrKg2cFVSceBEFL4RtmKrfhRcMWzyvq4yh4PwikZ` and permission `CAjEn5BnHunwprwxHSoANJLCGBDkMt1YKTeqnbtVi9bP`; both are absent on Devnet. An unsigned bootstrap simulation creates the exact version-2 shell and permission without touching either delegated Deposit or moving USDC; it consumes 27,814 compute units and estimates 5,430,440 lamports for fee plus rent.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH SETTLEMENT BOOTSTRAP SIM PASS | Six-call correction is deployed; old escrow is recovered; fresh settlement shell/permission creation passes unsigned simulation | Create and delegate the fresh settlement fixture, then prove settlement and expiry live |
...
8. **In progress:** the fresh acknowledged-payment fixture's unsigned bootstrap simulation passes. Next, separately approve its sender-signed base-Devnet creation, delegate it, then prove acknowledgement, automatic settlement, and recipient claim. Repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
