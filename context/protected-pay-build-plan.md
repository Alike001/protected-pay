...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY SIM PASS | Atomic private escrow and Payment-only scheduling finalized; six-call correction is deployed; atomic recovery passes authenticated signed simulation | Separately broadcast the existing-escrow recovery, then prove both corrected live terminal paths |
...
7. **In progress:** the authenticated, signature-verified atomic `advance_payment` plus `claim_payment` recovery simulation passes without persistence. Next, separately approve its fresh signature and Private ER broadcast.
