...
- The deterministic version-2.1 expiry fixture uses Payment ID `759c4e3fbf3ad9a11c1797e1db3f7e27d0cc4110956e07e0d10aaf1dd005b4a0`, Payment `AvZwmKkHPvrTHk3qyYCeuTAg2jSSrYM9tLEm4gKUD759`, and permission `Acg1YJySxHLLFwiFmQ4u5s3EpphtX2emGFGGKiPXAnPw`. Its unsigned Devnet bootstrap simulation passed with an empty version-2 shell, both existing Deposits untouched, no USDC movement, and estimated fee plus rent of 5,430,440 lamports. It remains unsigned and unbroadcast.
...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / EXPIRY BOOTSTRAP SIMULATION PASS | Corrected settlement is complete; distinct expiry shell/permission construction passes unsigned simulation | Separately approve expiry bootstrap, then prove unattended expiry and sender recovery live |
...
8. **In progress:** the corrected settlement path is complete. A separate deterministic expiry fixture now passes unsigned bootstrap simulation with no value movement. Next, separately approve its base-Devnet shell/permission creation, simulate and approve delegation, privately open it with the six-call schedule but no acknowledgement, then prove automatic expiry and sender recovery with retries idempotent.
