...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / EXPIRY OPEN-SCHEDULE SIM PASS | Distinct expiry fixture is delegated; sender-authenticated atomic private open plus six-call schedule passes signed simulation without persistence | Approve its live atomic broadcast, omit acknowledgement, then prove unattended expiry and sender recovery |
...
8. **In progress:** the corrected settlement path is complete. A separate deterministic expiry fixture is delegated, and sender-authenticated atomic private open plus six-call schedule passes signature-verified simulation without persistence. Next, separately approve the live atomic broadcast, omit acknowledgement, observe automatic expiry at call six, let only the sender recover escrow, and prove retries are idempotent.
