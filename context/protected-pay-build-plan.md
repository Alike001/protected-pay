...
Status: active implementation plan. G0–G3 and Phase 4 are complete with narrowed privacy language. Version 2.2 is deployed and byte-verified on Devnet. The React/Vite product has exercised real browser-wallet funding, protected send, sender Undo, unattended expiry/recovery, recipient acknowledgement, unattended settlement/claim, bounded Session Tokens, explicit revocation, and sanitized receipt restoration. The public repository and Vercel deployment are live. Phase 5 has a conditional verification pass and needs only the remaining live wrong-wallet denial for a full pass. Phase 6 presentation polish and dead-end navigation repair are complete; the MagicBlock Build submission packet and demo script are drafted, while the final video, authenticated portal confirmation, and submission remain.
...
- Fiat settlement and Resolva integration remain deferred. Session-key authorization is deployed on Devnet and remains explicitly limited to the one-hour program-scoped demonstration policy.
...
| G3 privacy | NARROW PASS | Outsider denial and the corrected Payment's full pre-delegation, private, and redacted public closeout boundary measured | Independently verify TEE hardware attestation before making stronger privacy claims |
...
| Phase 5 | CONDITIONAL PASS | React/Vite product, Wallet Standard connection, retry-safe custody/payment workflows, deployed one-hour MagicBlock Session Tokens, revocation, exact acknowledgement/claim receipts, all three core browser outcomes, and deterministic wrong-wallet concealment | Run the final live wrong-wallet denial with a third browser wallet |
| Phase 6 | IN PROGRESS | Public GitHub repository, green CI, Vercel production deployment, direct-route QA, judge-ready landing page, explicit home navigation, MagicBlock form copy, and a 90-second demo script | Capture the live third-wallet denial, record video, confirm the authenticated Blitz selector, and submit through MagicBlock Build |
...
2. **Complete locally for the core lifecycle and session authorization:** version-2 tests cover claims, cancel-versus-Crank, expiry boundaries, duplicate execution, overflow atomicity, legacy-version rejection, and conservation. One-hour Session Tokens now gate every user-authorized Private ER instruction, keep custody/public setup wallet-only, store the temporary key only in memory, and support explicit on-chain revocation. Policy-level automation pause remains deferred.
...
10. **Complete locally:** the four public Payment setup/delegation instructions remain a 797-byte transaction. Session-aware atomic private open/schedule is 626 bytes and retains the exact six-call cadence. A cold funded sender sees two public transaction approvals—bounded Session Token plus Payment setup—and one Query Filtering login message; the private open/schedule is signed silently by the in-memory session key. With a live session, only the Payment setup needs Phantom. Do not claim one-click or one-approval sending.
11. **Conditional pass:** the product UI covers its public landing page, connect, protected-balance unlock/read, first and subsequent test-USDC funding, retry-safe withdrawal, real funded-sender send/share/Undo/expired recovery, authoritative recipient Payment read/countdown, first-time recipient onboarding, real acknowledgement/claim, encrypted link-carried notes, cross-tab recovery, activity layout, judge proof, bounded-session expiry, and explicit revocation. Version 2.2 is deployed, the three core outcomes have run through the browser, and the exact recipient receipts are independently verified. Deterministic wrong-wallet QA now removes private values from the rendered DOM and disables action on desktop/mobile. One fresh live third-wallet denial remains before the audit can move from conditional pass to pass.
    - Local rendered QA now passes rejected signature, delayed confirmation, disconnected wallet, expired authentication, network interruption, and concurrent-tab fixtures at 1280×900 plus a 390×667 short-mobile recovery viewport. Every dialog closes and reopens correctly, technical details disclose on demand, the short-mobile backdrop scrolls, and no application warning/error or framework overlay was observed.
    - The gated live-wallet sequence is recorded in `artifacts/ui/p5-live-browser-wallet-checklist.md`; it starts with connection and non-broadcast rejection/disconnection/cross-tab evidence before any test-USDC transaction is authorized.
12. **In progress:** the public repository is `Alike001/protected-pay`, GitHub CI passes, and the production client is live at `https://protected-pay-azure.vercel.app` with `/`, `/app`, and `/pay` direct-route QA passing. The landing page now presents the problem, live flow, privacy boundary, verified build facts, source, and Devnet proof in one compact judge-facing narrative; the logo and visible back controls return from sender and recipient surfaces to `/`, and the Settings drawer is interaction-tested. MagicBlock Build form copy and the product-first 90-second demo script are drafted. The Luma event metadata ends at `2026-09-11T10:00:00Z`, while the logged-out Build portal reports no open event; the authenticated selector or organizer must confirm whether a late/final submission is still accepted. Next capture the live third-wallet denial, record the video, fill participant details, and submit through the MagicBlock Build portal if it remains available.
