...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / EXPIRY BOOTSTRAP LIVE | Corrected settlement is complete; distinct expiry shell and permission are finalized on Devnet without value movement | Simulate and approve expiry-fixture delegation, then prove unattended expiry and sender recovery live |
...
8. **In progress:** the corrected settlement path is complete. A separate deterministic expiry shell and permission are finalized on base Devnet with no value movement. Next, simulate and separately approve their delegation, privately open the Payment with the six-call schedule but no acknowledgement, then prove automatic expiry and sender recovery with retries idempotent.
