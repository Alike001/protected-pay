...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH PAYMENT SETTLED / CLAIM SIMULATION PASS | Corrected Crank schedule autonomously settled the acknowledged Payment; recipient-only claim and redaction simulate correctly | Separately approve claim broadcast, then prove fresh expiry live |
...
8. **In progress:** the corrected six-call schedule autonomously advanced the live Payment `Acknowledged -> Settled`, and recipient claim simulation credits 1 test USDC and redacts the Payment without persistence. Next, separately approve a fresh claim simulation and broadcast. Then repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
