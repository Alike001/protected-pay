...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / CORRECTED SETTLEMENT E2E PASS / LIVE AUTONOMOUS EXPIRY PASS | Distinct expiry fixture completed six validator-signed calls and changed `Created -> Expired` exactly at `+300s` without acknowledgement | Prove sender-only recovery, retry idempotency, then repeat privacy/commit checks |
...
8. **In progress:** the corrected settlement path is complete, and a separate deterministic expiry fixture autonomously reached `Expired` at the sixth validator-signed execution with no acknowledgement. Its sender-only claim passes unsigned public preflight. Next, authenticate the sender and run a signed non-broadcast simulation; after separate broadcast approval, prove the 1 test-USDC refund plus terminal redaction and verify retries cannot refund or mutate twice.
