...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / STALLED RECOVERY PASS / FRESH SETTLEMENT SIGNED SIMULATION PASS | Fresh Payment is delegated; authenticated atomic open plus six-call scheduling passes without persistence | Separately approve broadcast, then prove acknowledgement, settlement, claim, and expiry live |
...
8. **In progress:** the fresh acknowledged-payment shell and permission are delegated on Devnet, and sender-authenticated signed simulation proves atomic private open plus the corrected six-call schedule without persistence. Next, separately approve a fresh simulation and broadcast, immediately authenticate and acknowledge as the recipient, observe automatic settlement, and claim. Repeat on another fresh fixture for automatic expiry and sender recovery, with retries idempotent.
