...
| Phase 4 | V1 LIVE RECOVERY PASS / V2.1 DEPLOYED / BOTH E2E PATHS PASS / RETRY SAFETY PASS / PRIVACY PREFLIGHT PASS | Settlement, unattended expiry, owner-only recovery, terminal no-op, duplicate-claim rejection, and corrected-layout privacy boundary proven against live state | Authenticate and simulate the terminal Payment-only commit/undelegate, then gate its broadcast separately |
...
9. **IN PROGRESS:** The corrected-layout public and unauthenticated privacy audit passes, and the exact 320-byte Payment-only closeout transaction has been constructed with a no-op signer. Sender, recipient, payment ID, mint, permission membership, validator, delegation metadata, and timing are public; pending amount, memo hash, live status/deadlines, task ID, and aggregate balances remain private. Next authenticate the sender and run signature-verified simulation; only after that succeeds may a separate approval authorize broadcast and public post-commit verification.
