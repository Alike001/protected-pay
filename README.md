...
The corrected Payment layout has now passed its field-by-field public and unauthenticated privacy audit. Sender, recipient, payment ID, mint, permission membership, validator, and delegation timing are public; amount, memo hash, live status and deadlines, task ID, and aggregate balances remain protected while delegated. A 320-byte terminal closeout transaction has been constructed without a real signer. It commits only the already-redacted Payment, moves no USDC, and leaves both aggregate Deposits delegated. Authenticated signed simulation and then a separately approved broadcast are the final Phase 4 checks before UI work.
